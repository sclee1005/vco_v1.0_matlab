function [D,S,Q,PD] = perform_fast_marchingModifiedForVCO(W, start_points, options)
...
mode = getoptions(options, 'mode', 'vesselness');
...
if d==2
    switch mode
        case 'vesselness'
            [D,S,Q,PD] = perform_front_propagation_2d_addpd(W,start_points-1,end_points-1,nb_iter_max, H, L, values);
        case 'intensity'
            [D,S,Q,PD] = perform_front_propagation_2d_intensity(W,start_points-1,end_points-1,nb_iter_max, H, L, values);
        case 'gradient'
            [D,S,Q,PD] = perform_front_propagation_2d_gradient(W,start_points-1,end_points-1,nb_iter_max, H, L, values);
        case 'edge'
            [D,S,Q,PD] = perform_front_propagation_2d_intensity(W,start_points-1,end_points-1,nb_iter_max, H, L, values);
        otherwise            
    end
...
%     else
%         [D,D1,S] = my_perform_front_propagation_2d_slow(W,start_points,end_points,nb_iter_max, H);
%         Q = [];
%     end
% elseif d==3
%     [D,S,Q] = perform_front_propagation_3d(W,start_points-1,end_points-1,nb_iter_max, H, L, values);
