...
%
%   [D,S,Q,PD] = perform_front_propagation_2d_gradient(W,start_points,end_points,nb_iter_max, H, L, values);
%
...
#include "perform_front_propagation_2d_gradient.h"
#include "fheap/fib.h"
...
void perform_front_propagation_2d_gradient(T_callback_intert_node callback_insert_node)
{
	...
	while( !fh_isempty(open_heap) && num_iter<nb_iter_max && !stop_iteration )
	{
		...
		for( int k=0; k<4; ++k )
		{
			...
			if( ii>=0 && jj>=0 && ii<n && jj<p )
			{
				...
				if( ii<n-1 )
				{
                    a1 = D_(ii+1,jj) + pow(abs(W_(ii,jj)-W_(ii+1,jj)),n_power);
					//a1 = D_(ii+1,jj) + (W_(ii,jj)-W_(ii+1,jj))*(W_(ii,jj)-W_(ii+1,jj)); // ^2
                    //a1 = D_(ii+1,jj) + abs(W_(ii,jj)-W_(ii+1,jj)); // SCLEE: intensity gradient
					k1 = Q_(ii+1,jj);
					pd1 = PD_(ii+1,jj); // SCLEE: distance in pixels
				}
				if( ii>0 )
				{
                    double a2 = D_(ii-1,jj) + pow(abs(W_(ii,jj)-W_(ii-1,jj)),n_power);
					//double a2 = D_(ii-1,jj) + (W_(ii,jj)-W_(ii-1,jj))*(W_(ii,jj)-W_(ii-1,jj)); // ^2
                    //double a2 = D_(ii-1,jj) + abs(W_(ii,jj)-W_(ii-1,jj)); // SCLEE: intensity gradient
					if( a2<a1 ) {
						k1 = Q_(ii-1,jj);
						pd1 = PD_(ii-1,jj);// SCLEE: distance in pixels
						a1 = a2;
				...
				if( jj<p-1 )
				{
                    double a3 = D_(ii,jj+1) + pow(abs(W_(ii,jj)-W_(ii,jj+1)),n_power);
					//double a3 = D_(ii,jj+1) + (W_(ii,jj)-W_(ii,jj+1))*(W_(ii,jj)-W_(ii,jj+1)); // ^2
                    //double a3 = D_(ii,jj+1) + abs(W_(ii,jj)-W_(ii,jj+1)); // SCLEE: intensity gradient
					if( a3<a1 ) {
						k1 = Q_(ii,jj+1);
				...
				if( jj>0 )
				{
                    double a4 = D_(ii,jj-1) + pow(abs(W_(ii,jj)-W_(ii,jj-1)),n_power);
					//double a4 = D_(ii,jj-1) + (W_(ii,jj)-W_(ii,jj-1))*(W_(ii,jj)-W_(ii,jj-1)); // ^2
                    //double a4 = D_(ii,jj-1) + abs(W_(ii,jj)-W_(ii,jj-1)); // SCLEE: intensity gradient
					if( a4<a1 ) {
						k1 = Q_(ii,jj-1);
						pd1 = PD_(ii,jj-1);// SCLEE: distance in pixels
						a1 = a4;
